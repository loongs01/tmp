...
//                    dtToValues.get(dt);  // 直接获取 test
            ...
            if (overwritePartition && !clearedPartitions.contains(dt)) {
                try {
                    ...
                    String checkPartitionSql = String.format(
                            "SHOW PARTITIONS %s.%s PARTITION (dt='%s')",
                            databaseName, tableName, escapeSql(dt));
                    List<Map<String, Object>> partitions = hiveUtil.executeQuery(checkPartitionSql);
                    ...
                    if (partitions != null && !partitions.isEmpty()) {
                        ...
                        clearedPartitions.add(dt);  // 标记已删除该分区
                        System.out.println("Dropped existing partition dt=" + dt + " for table " + databaseName + "." + tableName);
                    } else {
                        System.out.println("Partition dt=" + dt + " does not exist. Skipping DROP.");
                    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        String outputPath = "/user/hive/warehouse/ods.db/" + args[1] + "/dt=" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd"));
        ...
        job.setJarByClass(WordSplitMR.class);
        job.setMapperClass(WordSplitMR.WordSplitMapper.class);
        job.setReducerClass(WordSplitMR.WordSplitReducer.class);
