...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的用户社交关系分析师，擅长从对话中提取用户的社交关系信息。
        请从以下聊天记录中提取如下字段信息，如果某项不存在请返回 None。
        格式为 JSON 对象：
        {{
            "family_member_id": "家人ID",
            "family_member_name": "家人姓名",
            "friend_id": "朋友ID",
            "friend_name": "朋友姓名",
            "colleague_id": "同事ID",
            "colleague_name": "同事姓名"
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            for field in [
                'family_member_id', 'family_member_name', 'friend_id', 'friend_name', 'colleague_id', 'colleague_name']:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    fields = [
        'user_id', 'stat_date', 'user_name', 'family_member_id', 'family_member_name', 'friend_id', 'friend_name', 'colleague_id', 'colleague_name'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_social_relationships_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        colleague_name = VALUES(colleague_name),
                        updated_time = NOW()
                """
