...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的用户事件分析师，擅长从对话中提取用户人生事件信息。
        请从以下聊天记录中提取如下字段信息，如果某项不存在请返回 None。
        格式为 JSON 对象：
        {{
            "event_id": "事件唯一标识",
            "event_type": "事件类型(如: 结婚, 生子, 购房, 升学, 离职)",
            "event_date": "事件发生日期(YYYY-MM-DD)",
            "event_description": "事件描述",
            "related_persons": "相关人员(如: 配偶姓名, 孩子姓名)",
            "event_location": "事件发生地点",
            "event_importance_score": "事件重要性评分(1-10)",
            "user_age_at_event": "事件发生时用户年龄",
            "user_gender": "性别(M-男, F-女, U-未知)",
            "user_region": "地区",
            "post_event_behavior_change": "事件后用户行为变化(如: 消费增加, 职业转变)",
            "financial_impact_amount": "事件对财务的影响金额(元, 正值表示收入增加, 负值表示支出增加)",
            "emotional_impact_description": "事件对情感的影响(如: 开心, 压力大)"
        }}
    ...
    def _parse_api_response(self, api_response: Dict[str, Any]) -> Dict[str, Any]:
        default_response = {
            ...
            "related_persons": None,
            "event_location": None,
            "event_importance_score": None,
            "user_age_at_event": None,
            "user_gender": None,
            "user_region": None,
            "post_event_behavior_change": None,
            "financial_impact_amount": None,
            "emotional_impact_description": None
        }
        try:
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            for field in [
                'event_id', 'event_type', 'event_date', 'event_description', 'related_persons',
                'event_location', 'event_importance_score', 'user_age_at_event', 'user_gender',
                'user_region', 'post_event_behavior_change', 'financial_impact_amount', 'emotional_impact_description']:
                profile[field] = merge_values(profile[field], analysis.get(field))
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    fields = [
        'user_id', 'stat_date', 'user_name', 'event_id', 'event_type', 'event_date', 'event_description', 'related_persons',
        'event_location', 'event_importance_score', 'user_age_at_event', 'user_gender', 'user_region', 'post_event_behavior_change',
        'financial_impact_amount', 'emotional_impact_description'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        related_persons = VALUES(related_persons),
                        event_location = VALUES(event_location),
                        event_importance_score = VALUES(event_importance_score),
                        user_age_at_event = VALUES(user_age_at_event),
                        user_gender = VALUES(user_gender),
                        user_region = VALUES(user_region),
                        post_event_behavior_change = VALUES(post_event_behavior_change),
                        financial_impact_amount = VALUES(financial_impact_amount),
                        emotional_impact_description = VALUES(emotional_impact_description),
                        updated_time = NOW()
                """
