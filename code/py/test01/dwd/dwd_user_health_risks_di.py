...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的健康风险分析师，擅长从对话中提取用户的健康风险相关信息。
        请从以下聊天记录中提取以下健康风险相关信息，如果某个信息不存在，请返回None。
        格式为JSON对象：
        {{
            "user_id": "用户id",
            "stat_date": "统计日期，格式为YYYY-MM-DD",
            "user_name": "用户姓名",
            "is_high_blood_pressure": "是否高血压风险（0/1）",
            "is_heart_disease_risk": "是否心脏病风险（0/1）",
            "is_obesity": "是否肥胖（0/1）",
            "is_sleep_disorder": "是否睡眠障碍（0/1）",
            "allergies": "过敏史",
    ...
    def _parse_api_response(self, api_response: Dict[str, Any]) -> Dict[str, Any]:
        default_response = {
            ...
            "stat_date": None,
            "user_name": None,
            "is_high_blood_pressure": None,
            "is_heart_disease_risk": None,
            "is_obesity": None,
            "is_sleep_disorder": None,
            "allergies": None,
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            profile['is_high_blood_pressure'] = merge_values(profile['is_high_blood_pressure'], analysis['is_high_blood_pressure'])
            profile['is_heart_disease_risk'] = merge_values(profile['is_heart_disease_risk'], analysis['is_heart_disease_risk'])
            profile['is_obesity'] = merge_values(profile['is_obesity'], analysis['is_obesity'])
            profile['is_sleep_disorder'] = merge_values(profile['is_sleep_disorder'], analysis['is_sleep_disorder'])
            profile['allergies'] = merge_values(profile['allergies'], analysis['allergies'])
            profile['chronic_diseases'] = merge_values(profile['chronic_diseases'], analysis['chronic_diseases'])
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    ...
    fields = [
        'user_id', 'stat_date', 'user_name', 'is_high_blood_pressure', 'is_heart_disease_risk', 'is_obesity',
        'is_sleep_disorder', 'allergies', 'chronic_diseases'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_health_risks_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        is_high_blood_pressure = VALUES(is_high_blood_pressure),
                        is_heart_disease_risk = VALUES(is_heart_disease_risk),
                        is_obesity = VALUES(is_obesity),
                        is_sleep_disorder = VALUES(is_sleep_disorder),
