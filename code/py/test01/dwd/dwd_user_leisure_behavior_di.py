...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的用户休闲娱乐行为分析师，擅长从对话中提取用户的休闲娱乐行为信息。
        请从以下聊天记录中提取以下信息，如果某个信息不存在，请返回None。
        格式为JSON对象：
        {{
            "age_group": "年龄组(如: 18-24 25-34)",
            "gender": "性别(M-男 F-女 U-未知)",
            "region": "地区",
            "daily_leisure_time": "日均休闲娱乐时长(分钟)",
            "weekly_leisure_days": "每周休闲娱乐天数",
            "leisure_activity_types": "休闲活动类型(如: 电影,游戏,阅读,运动,旅游)",
            "preferred_activity": "首选休闲活动",
            "leisure_activity_frequency": "休闲活动频率(如: 每天,每周,每月)"
        }}
    ...
    def _parse_api_response(self, api_response: Dict[str, Any]) -> Dict[str, Any]:
        default_response = {
            ...
            "gender": None,
            "region": None,
            "daily_leisure_time": None,
            "weekly_leisure_days": None,
            "leisure_activity_types": None,
            "preferred_activity": None,
            "leisure_activity_frequency": None
        }
        try:
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            profile['age_group'] = merge_values(profile['age_group'], analysis['age_group'])
            profile['gender'] = merge_values(profile['gender'], analysis['gender'])
            profile['region'] = merge_values(profile['region'], analysis['region'])
            profile['daily_leisure_time'] = merge_values(profile['daily_leisure_time'], analysis['daily_leisure_time'])
            profile['weekly_leisure_days'] = merge_values(profile['weekly_leisure_days'], analysis['weekly_leisure_days'])
            profile['leisure_activity_types'] = merge_values(profile['leisure_activity_types'], analysis['leisure_activity_types'])
            profile['preferred_activity'] = merge_values(profile['preferred_activity'], analysis['preferred_activity'])
            profile['leisure_activity_frequency'] = merge_values(profile['leisure_activity_frequency'], analysis['leisure_activity_frequency'])
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    ...
    fields = [
        'user_id', 'stat_date', 'user_name', 'age_group', 'gender', 'region', 'daily_leisure_time', 'weekly_leisure_days',
        'leisure_activity_types', 'preferred_activity', 'leisure_activity_frequency'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_leisure_behavior_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        daily_leisure_time = VALUES(daily_leisure_time),
                        weekly_leisure_days = VALUES(weekly_leisure_days),
                        leisure_activity_types = VALUES(leisure_activity_types),
                        preferred_activity = VALUES(preferred_activity),
                        leisure_activity_frequency = VALUES(leisure_activity_frequency),
                        updated_time = NOW()
                """
