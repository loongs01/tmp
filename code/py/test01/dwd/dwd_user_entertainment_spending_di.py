...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的用户娱乐消费分析师，擅长从对话中提取用户的娱乐消费信息。
        请从以下聊天记录中提取以下信息，如果某个信息不存在，请返回None。
        格式为JSON对象：
        {{
            "monthly_leisure_spending": "月均休闲娱乐消费(元)",
            "spending_categories": "消费类别(如: 电影票,游戏充值,书籍,运动装备)",
            "preferred_payment_method": "首选支付方式(如: 支付宝,微信,信用卡)"
        }}
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            profile['monthly_leisure_spending'] = merge_values(profile['monthly_leisure_spending'], analysis['monthly_leisure_spending'])
            profile['spending_categories'] = merge_values(profile['spending_categories'], analysis['spending_categories'])
            profile['preferred_payment_method'] = merge_values(profile['preferred_payment_method'], analysis['preferred_payment_method'])
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    ...
    fields = [
        'user_id', 'stat_date', 'user_name', 'monthly_leisure_spending', 'spending_categories', 'preferred_payment_method'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_entertainment_spending_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        monthly_leisure_spending = VALUES(monthly_leisure_spending),
