...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的用户性格分析师，擅长从对话中提取用户的性格特征。
        请从以下聊天记录中提取如下字段信息，如果某项不存在请返回 None。
        格式为 JSON 对象：
        {{
            "personality_keywords": "性格关键词(逗号分隔)",
            "personality_overview": "性格概览",
            "personality_strengths": "性格优点",
            "personality_weaknesses": "性格缺点",
            "strengths_suggestions": "扬长建议",
            "weaknesses_suggestions": "避短建议"
        }}
    ...
    def _parse_api_response(self, api_response: Dict[str, Any]) -> Dict[str, Any]:
        default_response = {
            ...
            "personality_overview": None,
            "personality_strengths": None,
            "personality_weaknesses": None,
            "strengths_suggestions": None,
            "weaknesses_suggestions": None
        }
        try:
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            for field in [
                'personality_keywords', 'personality_overview', 'personality_strengths', 'personality_weaknesses', 'strengths_suggestions', 'weaknesses_suggestions']:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    fields = [
        'user_id', 'stat_date', 'user_name', 'personality_keywords', 'personality_overview', 'personality_strengths',
        'personality_weaknesses', 'strengths_suggestions', 'weaknesses_suggestions'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_personality_analysis_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        personality_keywords = VALUES(personality_keywords),
                        personality_overview = VALUES(personality_overview),
                        personality_strengths = VALUES(personality_strengths),
                        personality_weaknesses = VALUES(personality_weaknesses),
                        strengths_suggestions = VALUES(strengths_suggestions),
                        weaknesses_suggestions = VALUES(weaknesses_suggestions),
                        updated_time = NOW()
                """
