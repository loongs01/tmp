...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的健康数据分析师，擅长从对话中提取用户的运动与健康相关指标。
        请从以下聊天记录中提取以下运动健康相关信息，如果某个信息不存在，请返回None。
        格式为JSON对象：
        {{
            "user_id": "用户id",
            ...
            "total_calories": "总消耗卡路里(千卡)",
            "total_exercise_minutes": "总运动时长(分钟)",
            "avg_sleep_duration": "平均睡眠时长(小时)",
            "sleep_quality_score": "睡眠质量评分(1-100)"
        }}
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            profile['total_steps'] = merge_values(profile['total_steps'], analysis['total_steps'])
            profile['avg_daily_steps'] = merge_values(profile['avg_daily_steps'], analysis['avg_daily_steps'])
            profile['total_calories'] = merge_values(profile['total_calories'], analysis['total_calories'])
            profile['total_exercise_minutes'] = merge_values(profile['total_exercise_minutes'],
                                                            analysis['total_exercise_minutes'])
            profile['avg_sleep_duration'] = merge_values(profile['avg_sleep_duration'], analysis['avg_sleep_duration'])
            profile['sleep_quality_score'] = merge_values(profile['sleep_quality_score'], analysis['sleep_quality_score'])
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    ...
    fields = [
        'user_id', 'stat_date', 'user_name', 'total_steps', 'avg_daily_steps', 'total_calories',
        'total_exercise_minutes', 'avg_sleep_duration', 'sleep_quality_score'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_exercise_health_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        avg_daily_steps = VALUES(avg_daily_steps),
                        total_calories = VALUES(total_calories),
                        total_exercise_minutes = VALUES(total_exercise_minutes),
                        avg_sleep_duration = VALUES(avg_sleep_duration),
                        sleep_quality_score = VALUES(sleep_quality_score),
                        updated_time = NOW()
                """
