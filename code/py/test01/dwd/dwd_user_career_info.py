...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的职业信息分析师，擅长从对话中提取用户的职业相关信息。
        请从以下聊天记录中提取以下职业相关信息，如果某个信息不存在，请返回None。
        格式为JSON对象：
        {{
            "user_id": "用户id",
            "stat_date": "统计日期，格式为YYYY-MM-DD",
            "user_name": "用户姓名",
            "industry_name": "行业名称（国标）",
            "occupation_name": "职业名称（国标）",
            "position_name": "职位名称",
            "company_name": "公司名称",
            "company_size": "公司规模(如: 小型 中型 大型)",
            "work_experience_years": "工作年限(年)",
            "current_salary": "当前薪资(元)",
            "job_hopping_frequency": "跳槽频率(1-低 2-中 3-高)",
            "career_satisfaction_score": "职业满意度(1-10分)",
            "has_attended_training": "是否参加过培训(0-否 1-是)",
            "first_job_date": "首次工作日期，格式为YYYY-MM-DD"
        }}
    ...
    def _parse_api_response(self, api_response: Dict[str, Any]) -> Dict[str, Any]:
        default_response = {
            ...
            "current_salary": None,
            "job_hopping_frequency": None,
            "career_satisfaction_score": None,
            "has_attended_training": None,
            "first_job_date": None
        }
        try:
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            profile['industry_name'] = merge_values(profile['industry_name'], analysis['industry_name'])
            profile['occupation_name'] = merge_values(profile['occupation_name'], analysis['occupation_name'])
            profile['position_name'] = merge_values(profile['position_name'], analysis['position_name'])
            profile['company_name'] = merge_values(profile['company_name'], analysis['company_name'])
            profile['company_size'] = merge_values(profile['company_size'], analysis['company_size'])
            profile['work_experience_years'] = merge_values(profile['work_experience_years'], analysis['work_experience_years'])
            profile['current_salary'] = merge_values(profile['current_salary'], analysis['current_salary'])
            profile['job_hopping_frequency'] = merge_values(profile['job_hopping_frequency'], analysis['job_hopping_frequency'])
            profile['career_satisfaction_score'] = merge_values(profile['career_satisfaction_score'], analysis['career_satisfaction_score'])
            profile['has_attended_training'] = merge_values(profile['has_attended_training'], analysis['has_attended_training'])
            profile['first_job_date'] = merge_values(profile['first_job_date'], analysis['first_job_date'])
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    ...
    fields = [
        'user_id', 'stat_date', 'user_name', 'industry_name', 'occupation_name', 'position_name', 'company_name',
        'company_size', 'work_experience_years', 'current_salary', 'job_hopping_frequency', 'career_satisfaction_score',
        'has_attended_training', 'first_job_date'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_career_info (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        job_hopping_frequency = VALUES(job_hopping_frequency),
                        career_satisfaction_score = VALUES(career_satisfaction_score),
                        has_attended_training = VALUES(has_attended_training),
                        first_job_date = VALUES(first_job_date),
                        updated_time = NOW()
                """
