...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的健康调理建议分析师，擅长从对话中提取用户的调理方法建议。
        请从以下聊天记录中提取如下字段信息，如果某项不存在请返回 None。
        格式为 JSON 对象：
        {{
            "diet_recommendations": "饮食调理建议",
            "herbal_recommendations": "中药调理建议",
            "lifestyle_recommendations": "生活方式建议",
            "acupoint_recommendations": "穴位调理建议",
            "other_recommendations": "其他调理建议"
        }}
    ...
    def _parse_api_response(self, api_response: Dict[str, Any]) -> Dict[str, Any]:
        default_response = {
            ...
            "herbal_recommendations": None,
            "lifestyle_recommendations": None,
            "acupoint_recommendations": None,
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            for field in [
                'diet_recommendations', 'herbal_recommendations', 'lifestyle_recommendations', 'acupoint_recommendations', 'other_recommendations']:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    fields = [
        'user_id', 'stat_date', 'user_name', 'diet_recommendations', 'herbal_recommendations', 'lifestyle_recommendations', 'acupoint_recommendations', 'other_recommendations'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_treatment_methods_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        diet_recommendations = VALUES(diet_recommendations),
                        herbal_recommendations = VALUES(herbal_recommendations),
                        lifestyle_recommendations = VALUES(lifestyle_recommendations),
                        acupoint_recommendations = VALUES(acupoint_recommendations),
                        other_recommendations = VALUES(other_recommendations),
                        updated_time = NOW()
                """
