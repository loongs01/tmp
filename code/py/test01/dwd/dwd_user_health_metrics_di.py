...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的健康数据分析师，擅长从对话中提取用户的健康指标信息。
        请从以下聊天记录中提取以下健康相关信息，如果某个信息不存在，请返回None。
        格式为JSON对象：
        {{
            "user_id": "用户id",
            ...
            "max_heart_rate": "最大心率",
            "min_heart_rate": "最小心率",
            "avg_blood_pressure": "平均血压(如120/80)",
            "avg_blood_oxygen": "平均血氧饱和度(%)",
            "avg_body_temp": "平均体温(℃)"
        }}
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            profile['avg_heart_rate'] = merge_values(profile['avg_heart_rate'], analysis['avg_heart_rate'])
            profile['max_heart_rate'] = merge_values(profile['max_heart_rate'], analysis['max_heart_rate'])
            profile['min_heart_rate'] = merge_values(profile['min_heart_rate'], analysis['min_heart_rate'])
            profile['avg_blood_pressure'] = merge_values(profile['avg_blood_pressure'], analysis['avg_blood_pressure'])
            profile['avg_blood_oxygen'] = merge_values(profile['avg_blood_oxygen'], analysis['avg_blood_oxygen'])
            profile['avg_body_temp'] = merge_values(profile['avg_body_temp'], analysis['avg_body_temp'])
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    ...
    fields = [
        'user_id', 'stat_date', 'user_name', 'avg_heart_rate', 'max_heart_rate', 'min_heart_rate', 'avg_blood_pressure',
        'avg_blood_oxygen', 'avg_body_temp'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_health_metrics_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        avg_blood_pressure = VALUES(avg_blood_pressure),
                        avg_blood_oxygen = VALUES(avg_blood_oxygen),
                        avg_body_temp = VALUES(avg_body_temp),
                        updated_time = NOW()
                """
