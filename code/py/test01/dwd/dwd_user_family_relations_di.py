...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的家庭关系分析师，擅长从对话中提取用户的家庭关系相关信息。
        请从以下聊天记录中提取以下家庭关系相关信息，如果某个信息不存在，请返回None。
        格式为JSON对象：
        {{
            "user_id": "用户id",
            "stat_date": "统计日期，格式为YYYY-MM-DD",
            "user_name": "用户姓名",
            "family_member_id": "家庭成员id",
            "family_member_name": "家庭成员姓名",
            "relation_type": "关系类型(如: 父母 配偶 子女 兄弟姐妹)",
            "relation_description": "关系描述(如: 父亲 长子)",
            "age": "年龄",
            "gender": "性别(M-男 F-女 U-未知)",
            "occupation": "职业",
            "education_level": "教育程度(如: 高中 本科 硕士)",
            "marital_status": "婚姻状况(如: 已婚 未婚 离异)",
            "is_living_together": "是否同住(0-否 1-是)",
            "contact_frequency": "联系频率(如: 每天 每周 每月)",
            "last_contact_date": "最后联系日期，格式为YYYY-MM-DD",
            "is_financial_dependent": "是否经济依赖(0-否 1-是)",
            "health_status": "健康状况(如: 健康 慢性病 残疾)",
            "has_chronic_disease": "是否有慢性病(0-否 1-是)"
        }}
    ...
    def _parse_api_response(self, api_response: Dict[str, Any]) -> Dict[str, Any]:
        default_response = {
            ...
            "marital_status": None,
            "is_living_together": None,
            "contact_frequency": None,
            "last_contact_date": None,
            "is_financial_dependent": None,
            "health_status": None,
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            profile['family_member_id'] = merge_values(profile['family_member_id'], analysis['user_id'])
            profile['family_member_name'] = merge_values(profile['family_member_name'], analysis['user_name'])
            profile['relation_type'] = merge_values(profile['relation_type'], analysis['relation_type'])
            profile['relation_description'] = merge_values(profile['relation_description'], analysis['relation_description'])
            profile['age'] = merge_values(profile['age'], analysis['age'])
            profile['gender'] = merge_values(profile['gender'], analysis['gender'])
            profile['occupation'] = merge_values(profile['occupation'], analysis['occupation'])
            profile['education_level'] = merge_values(profile['education_level'], analysis['education_level'])
            profile['marital_status'] = merge_values(profile['marital_status'], analysis['marital_status'])
            profile['is_living_together'] = merge_values(profile['is_living_together'], analysis['is_living_together'])
            profile['contact_frequency'] = merge_values(profile['contact_frequency'], analysis['contact_frequency'])
            profile['last_contact_date'] = merge_values(profile['last_contact_date'], analysis['last_contact_date'])
            profile['is_financial_dependent'] = merge_values(profile['is_financial_dependent'], analysis['is_financial_dependent'])
            profile['health_status'] = merge_values(profile['health_status'], analysis['health_status'])
            profile['has_chronic_disease'] = merge_values(profile['has_chronic_disease'], analysis['has_chronic_disease'])
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    ...
    fields = [
        'user_id', 'stat_date', 'user_name', 'family_member_id', 'family_member_name', 'relation_type',
        'relation_description', 'age', 'gender', 'occupation', 'education_level', 'marital_status',
        'is_living_together', 'contact_frequency', 'last_contact_date', 'is_financial_dependent',
        'health_status', 'has_chronic_disease'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_family_relations_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        is_living_together = VALUES(is_living_together),
                        contact_frequency = VALUES(contact_frequency),
                        last_contact_date = VALUES(last_contact_date),
                        is_financial_dependent = VALUES(is_financial_dependent),
