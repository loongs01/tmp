...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的健康行为分析师，擅长从对话中提取用户的健康行为相关信息。
        请从以下聊天记录中提取以下健康行为相关信息，如果某个信息不存在，请返回None。
        格式为JSON对象：
        {{
            "user_id": "用户id",
            "stat_date": "统计日期，格式为YYYY-MM-DD",
            "user_name": "用户姓名",
            "exercise_frequency": "运动频率(1-不运动2-偶尔3-经常4-每天)",
            "diet_habit": "饮食习惯(1-不健康2-一般3-健康)",
            "health_management_flag": "是否进行健康管理（0/1）"
        }}
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            profile['exercise_frequency'] = merge_values(profile['exercise_frequency'], analysis['exercise_frequency'])
            profile['diet_habit'] = merge_values(profile['diet_habit'], analysis['diet_habit'])
            profile['health_management_flag'] = merge_values(profile['health_management_flag'], analysis['health_management_flag'])
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    ...
    fields = [
        'user_id', 'stat_date', 'user_name', 'exercise_frequency', 'diet_habit', 'health_management_flag'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_health_behaviors_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        diet_habit = VALUES(diet_habit),
                        health_management_flag = VALUES(health_management_flag),
                        updated_time = NOW()
                """
