...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的学习偏好分析师，擅长从对话中提取用户的学习偏好相关信息。
        请从以下聊天记录中提取以下学习偏好相关信息，如果某个信息不存在，请返回None。
        格式为JSON对象：
        {{
            "user_id": "用户id",
            "stat_date": "统计日期，格式为YYYY-MM-DD",
            "user_name": "用户姓名",
            "preferred_learning_method": "偏好学习方式(如: 在线课程 书籍 培训)",
            "preferred_subject": "偏好学习科目",
            "learning_goal": "学习目标"
        }}
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            profile['preferred_learning_method'] = merge_values(profile['preferred_learning_method'], analysis['preferred_learning_method'])
            profile['preferred_subject'] = merge_values(profile['preferred_subject'], analysis['preferred_subject'])
            profile['learning_goal'] = merge_values(profile['learning_goal'], analysis['learning_goal'])
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    ...
    fields = [
        'user_id', 'stat_date', 'user_name', 'preferred_learning_method', 'preferred_subject', 'learning_goal'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_learning_preference_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        preferred_learning_method = VALUES(preferred_learning_method),
                        preferred_subject = VALUES(preferred_subject),
