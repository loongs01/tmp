...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的学习行为分析师，擅长从对话中提取用户的学习行为相关信息。
        请从以下聊天记录中提取以下学习行为相关信息，如果某个信息不存在，请返回None。
        格式为JSON对象：
        {{
            "user_id": "用户id",
            "stat_date": "统计日期，格式为YYYY-MM-DD",
            "user_name": "用户姓名",
            "total_courses_completed": "完成课程总数",
            "avg_course_score": "平均课程得分",
            "total_learning_hours": "总学习时长(小时)",
            "avg_daily_learning_time": "日均学习时长(小时)",
            "learning_frequency": "学习频率(1-低 2-中 3-高)",
            "first_learning_date": "首次学习日期，格式为YYYY-MM-DD"
        }}
    ...
    def _parse_api_response(self, api_response: Dict[str, Any]) -> Dict[str, Any]:
        default_response = {
            ...
            "stat_date": None,
            "user_name": None,
            "total_courses_completed": None,
            "avg_course_score": None,
            "total_learning_hours": None,
            "avg_daily_learning_time": None,
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            profile['total_courses_completed'] = merge_values(profile['total_courses_completed'], analysis['total_courses_completed'])
            profile['avg_course_score'] = merge_values(profile['avg_course_score'], analysis['avg_course_score'])
            profile['total_learning_hours'] = merge_values(profile['total_learning_hours'], analysis['total_learning_hours'])
            profile['avg_daily_learning_time'] = merge_values(profile['avg_daily_learning_time'], analysis['avg_daily_learning_time'])
            profile['learning_frequency'] = merge_values(profile['learning_frequency'], analysis['learning_frequency'])
            profile['first_learning_date'] = merge_values(profile['first_learning_date'], analysis['first_learning_date'])
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    ...
    fields = [
        'user_id', 'stat_date', 'user_name', 'total_courses_completed', 'avg_course_score', 'total_learning_hours',
        'avg_daily_learning_time', 'learning_frequency', 'first_learning_date'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_learning_behavior_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        total_courses_completed = VALUES(total_courses_completed),
                        avg_course_score = VALUES(avg_course_score),
                        total_learning_hours = VALUES(total_learning_hours),
                        avg_daily_learning_time = VALUES(avg_daily_learning_time),
                        learning_frequency = VALUES(learning_frequency),
                        first_learning_date = VALUES(first_learning_date),
                        updated_time = NOW()
                """
