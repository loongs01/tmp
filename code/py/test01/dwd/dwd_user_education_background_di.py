...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的教育背景分析师，擅长从对话中提取用户的教育相关信息。
        请从以下聊天记录中提取以下教育背景相关信息，如果某个信息不存在，请返回None。
        格式为JSON对象：
        {{
            "user_id": "用户id",
            "stat_date": "统计日期，格式为YYYY-MM-DD",
            "user_name": "用户姓名",
            "education_level": "教育程度(如: 高中 本科 硕士 博士)",
            "major": "专业名称",
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            profile['major'] = merge_values(profile['major'], analysis['major'])
            profile['graduation_year'] = merge_values(profile['graduation_year'], analysis['graduation_year'])
            profile['school_name'] = merge_values(profile['school_name'], analysis['school_name'])
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    ...
    fields = [
        'user_id', 'stat_date', 'user_name', 'education_level', 'major', 'graduation_year', 'school_name'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_education_background_di (
