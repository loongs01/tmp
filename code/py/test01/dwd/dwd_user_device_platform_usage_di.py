...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的用户设备与平台使用分析师，擅长从对话中提取用户的设备与平台使用信息。
        请从以下聊天记录中提取以下信息，如果某个信息不存在，请返回None。
        格式为JSON对象：
        {{
            "preferred_devices": "常用设备(如: 手机,平板,电脑,电视)",
            "leisure_app_usage": "常用休闲娱乐应用(如: 腾讯视频,王者荣耀,豆瓣)",
            "daily_app_usage_time": "日均应用使用时长(分钟)"
        }}
    ...
    def _parse_api_response(self, api_response: Dict[str, Any]) -> Dict[str, Any]:
        default_response = {
            ...
            "leisure_app_usage": None,
            "daily_app_usage_time": None
        }
        try:
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            profile['preferred_devices'] = merge_values(profile['preferred_devices'], analysis['preferred_devices'])
            profile['leisure_app_usage'] = merge_values(profile['leisure_app_usage'], analysis['leisure_app_usage'])
            profile['daily_app_usage_time'] = merge_values(profile['daily_app_usage_time'], analysis['daily_app_usage_time'])
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    ...
    fields = [
        'user_id', 'stat_date', 'user_name', 'preferred_devices', 'leisure_app_usage', 'daily_app_usage_time'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_device_platform_usage_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        preferred_devices = VALUES(preferred_devices),
                        leisure_app_usage = VALUES(leisure_app_usage),
                        daily_app_usage_time = VALUES(daily_app_usage_time),
                        updated_time = NOW()
                """
