...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的技能水平分析师，擅长从对话中提取用户的技能水平相关信息。
        请从以下聊天记录中提取以下技能水平相关信息，如果某个信息不存在，请返回None。
        格式为JSON对象：
        {{
            "user_id": "用户id",
            "stat_date": "统计日期，格式为YYYY-MM-DD",
            "user_name": "用户姓名",
            "skill_level_coding": "技能水平(1-初级 2-中级 3-高级)",
            "certification_name": "获得证书名称"
        }}
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            profile['skill_level_coding'] = merge_values(profile['skill_level_coding'], analysis['skill_level_coding'])
            profile['certification_name'] = merge_values(profile['certification_name'], analysis['certification_name'])
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    ...
    fields = [
        'user_id', 'stat_date', 'user_name', 'skill_level_coding', 'certification_name'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_skill_level_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        skill_level_coding = VALUES(skill_level_coding),
