...
class UserProfileExtractor:
    ...
    def _construct_prompt(self, query: str, response: str) -> str:
        return f"""
        你是一个专业的用户社交互动分析师，擅长从对话中提取用户的社交互动信息。
        请从以下聊天记录中提取以下信息，如果某个信息不存在，请返回None。
        格式为JSON对象：
        {{
            "leisure_social_frequency": "休闲社交频率(如: 每天,每周,每月)",
            "preferred_social_platforms": "首选社交平台(如: 微信,微博,抖音)",
            "leisure_group_activities": "参与的休闲团体活动(如: 读书会,运动俱乐部)"
        }}
    ...
    def _parse_api_response(self, api_response: Dict[str, Any]) -> Dict[str, Any]:
        default_response = {
            "leisure_social_frequency": None,
            "preferred_social_platforms": None,
            "leisure_group_activities": None
        }
        try:
...
def analyze_user_interactions(user_id: str, extractor: UserProfileExtractor) -> Optional[Dict[str, Any]]:
    try:
        ...
        for interaction in interactions:
            ...
            profile['leisure_social_frequency'] = merge_values(profile['leisure_social_frequency'], analysis['leisure_social_frequency'])
            profile['preferred_social_platforms'] = merge_values(profile['preferred_social_platforms'], analysis['preferred_social_platforms'])
            profile['leisure_group_activities'] = merge_values(profile['leisure_group_activities'], analysis['leisure_group_activities'])
        return profile
    except Exception as e:
...
def save_user_profile(profile: Dict[str, Any]) -> bool:
    ...
    fields = [
        'user_id', 'stat_date', 'user_name', 'leisure_social_frequency', 'preferred_social_platforms', 'leisure_group_activities'
    ]
    try:
        with db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO dwd_user_social_interaction_di (
                    ...
                    ON DUPLICATE KEY UPDATE
                        ...
                        leisure_social_frequency = VALUES(leisure_social_frequency),
                        preferred_social_platforms = VALUES(preferred_social_platforms),
                        leisure_group_activities = VALUES(leisure_group_activities),
                        updated_time = NOW()
                """
