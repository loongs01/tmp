# Generated ETL script for sheet: dws_标准收入成本模型_业绩分配
...
CREATE_TABLE_SQL = r"""
CREATE TABLE IF NOT EXISTS `dws_fin_revenue_cost_alloc_df` (
  ...
  `sales_percent` VARCHAR(256) COMMENT '业绩分配比例',
  ...
  `parent_acc_code` VARCHAR(256) COMMENT '父级科目编码',
  `currency` VARCHAR(256) COMMENT '原币别',
  `local_currency` VARCHAR(256) COMMENT '本位币别',
...
def insert_into_starrocks(df):
    ...
    try:
        with conn.cursor() as cur:
            ...
            insert_sql = "INSERT INTO `{}` ({}) VALUES ({})".format("dws_fin_revenue_cost_alloc_df", COL_LIST, "%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s")
            values = []
