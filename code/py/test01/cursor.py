...
# --- 静态资源与词典 (扩展后) ---
DOMAIN_DICTS = {
    "food": {"口味": ["辣", "甜", "咸", "酸", "麻辣"], "中餐": ["川菜", "粤菜", "湘菜"], "食材": ["猪肉", "牛肉", "鸡肉", "海鲜"]},
    "hobby": {"游戏": ["PlayStation", "PC游戏", "手游"], "阅读": ["小说", "科幻", "历史"], "电影": ["动作片", "喜剧", "科幻片"]},
...
class UserProfileExtractor:
    ...
    EXTRACTION_CONFIG = {
        # 饮食偏好
        'favorite_foods': {'domain': 'food', 'patterns': [r'(?:喜欢|爱吃|偏好)(?:的|是)?\s*([^。，,；;、\n]+?)(?:菜|食物)?'], 'mode': 'noun_phrase'},
        'dietary_restrictions': {'domain': 'food', 'patterns': [r'(?:不吃|不能|忌口|对)(?:\s*是)?\s*([^。，,；;、\n]+?)(?:过敏)?'], 'mode': 'noun_phrase'},
        # 颜色偏好
        'favorite_colors': {'domain': 'color', 'patterns': [r'(?:喜欢|偏爱)的?颜色是(.+?)', r'喜欢(.+?色)'], 'mode': 'domain_dict'},
        # 兴趣爱好 (细分)
        'hobbies': {'domain': 'hobby', 'patterns': [r'我的爱好是(.+)', r'(?:平常|业余时间)?喜欢(.+?)[。，,]', r'对(.+?)感兴趣'], 'mode': 'noun_phrase'},
        'favorite_games': {'domain': 'hobby', 'patterns': [r'(?:喜欢玩|在玩|推荐|沉迷于)\s*《?([^》\n,]+?)》?(?:这款游戏)?'], 'mode': 'noun_phrase'},
        'favorite_books': {'domain': 'hobby', 'patterns': [r'(?:喜欢读|在看|推荐的书是)\s*《?([^》\n,]+?)》?'], 'mode': 'noun_phrase'},
        'favorite_movies': {'domain': 'hobby', 'patterns': [r'(?:喜欢看|推荐的电影是)\s*《?([^》\n,]+?)》?'], 'mode': 'noun_phrase'},
        # 个人发展
        'achievements': {'patterns': [r'(?:我|最近)?(?:完成|达成|实现|获得)了?\s*([^,。]+?)(?:的成就|的目标)?'], 'mode': 'raw'},
        ...
        # 旅行信息
        'visited_places': {'domain': 'travel', 'patterns': [r'去过(?!.+旅行)([^,。]+)', r'曾经在(.+?)旅行'], 'mode': 'noun_phrase'},
        'desired_travel_places': {'domain': 'travel', 'patterns': [r'(?:想|计划|打算)去(.+?)旅行', r'想去的地方是(.+)'], 'mode': 'noun_phrase'},
    }
    ...
    def is_meaningful(text: str) -> bool:
        ...
        if text_lower in {"none", "null", "无", "没有", "不详", "未知", "test", "测试", "一些", "一种", "那个", "这个"}: return False
    ...
    def merge_values(current: Optional[Any], new: Optional[Any], separator: str = ', ') -> Optional[Any]:
        ...
        def to_list(val):
            ...
            if isinstance(val, str): return [v.strip() for v in val.split(separator) if UserProfileExtractor.is_meaningful(v)]
        ...
        for item in new_list:
            if not any(UserProfileExtractor.semantic_similarity(item, exist) > SEMANTIC_SIM_THRESHOLD for exist in current_list):
    ...
    try:
        with db_connection() as conn:
            with conn.cursor(dictionary=True) as cursor:
                ...
                cursor.execute("SELECT query_text, response_text FROM ods_user_interaction_di WHERE user_id = %s ORDER BY interaction_time", (user_id,))
