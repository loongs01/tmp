-- drop Table if  exists dm_reelshort_data.dm_t82_reelshort_story_play_statistic_di;
...
-- ,PRIMARY KEY (id,etl_date,story_id)
-- )DISTRIBUTE BY HASH(etl_date,story_id) PARTITION BY VALUE(DATE_FORMAT(etl_date, '%Y%m')) LIFECYCLE 120 INDEX_ALL='Y' STORAGE_POLICY='HOT' ENGINE='XUANWU' TABLE_PROPERTIES='{"format":"columnstore"}' COMMENT='reelshort 书籍每日播放指标统计';

delete from dm_reelshort_data.dm_t82_reelshort_story_play_statistic_di where etl_date='${TX_DATE}';
insert into dm_reelshort_data.dm_t82_reelshort_story_play_statistic_di
select
      null                       as id                      -- 自增id
