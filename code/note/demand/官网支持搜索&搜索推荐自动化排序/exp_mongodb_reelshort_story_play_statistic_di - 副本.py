...
if __name__ == '__main__':
    ...
    sql = """select cast(etl_date as varchar) as etl_date
                        ,language_id
                        ,display_tag
                        ,story_id
                        ,lang
    ...
    # 将数据导出到mongodb
    print("sql: " + sql)
    handleMongo = HandleMongodb("test-rt", collection_name)
    myclient, mongoCol = handleMongo.conn_mongodb()
    # 删除 etl_date 小于 analysis_date7 的所有文档
    delete_result = mongoCol.delete_many({"etl_date": {"$lt": analysis_date7}})
    # 输出删除的文档数量
