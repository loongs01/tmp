...
select  t1.id
      ...
      -- ,nvl(t14.network,'') as network
      -- add by lcz
      -- ,nvl(cast(t6.trans_complete_amt as decimal(18,4)),0) as trans_complete_amt -- 订单支付金额
      ,nvl(cast(t6.trans_start_amt as decimal(18,4)),0)    as trans_start_amt -- 拉起支付sdk开始支付金额     
      ,nvl(t12.recent_visit_source,'')                     as recent_visit_source -- 最近一次访问来源
      ,nvl(t12.recent_visit_source_type,'')                as recent_visit_source_type -- 最近一次访问来源类型
...
select  uuid
      ...
      -- ,sum(case when  pay_status='pay_complete' then sku_price/100 else 0 end  ) as trans_complete_amt -- 订单支付金额 pay_amount
...
left join  (
  ...
  select uuid
       ...
       ,min(first_visit_landing_page)          as first_visit_landing_page          -- 首次访问着陆页
       ,min(first_visit_landing_page_story_id) as first_visit_landing_page_story_id -- 首次访问着陆页书籍id
       ,min(first_visit_landing_page_chap_id)  as first_visit_landing_page_chap_id  -- 首次访问着陆页章节id
  from dwd_data.dim_t99_reelshort_user_lt_info
  where etl_date between date_sub('${TX_DATE}',1) and '${TX_DATE}'
  and uuid in (select distinct uuid from dwd_data.dwd_t01_reelshort_user_info_di where etl_date='${TX_DATE}')
  group by uuid
)t11
on t1.uuid=t11.uuid
left join
...
left join (
      select uuid ,media_type,dlink_story_id -- ,t2.network
      ...
      -- left join (select distinct media_source,network from dim_data.dim_t99_appsflyer_adjust_network where app_name in ('reelshort') ) t2
      --   on t1.media_type=t2.media_source
      where rank=1
)t14
on t1.uuid=t14.uuid
;
