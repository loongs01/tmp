...
select
    ...
    -- add by lcz
    -- ,t4.is_free          as is_free           -- 1: 免费, 0:非免费(包括已解锁和未解锁)
    -- ,t3.search_story_ids as search_story_ids  -- 搜索结果页面中，搜索结果的书籍列表，元素结构是 书籍顺序#书籍ID#匹配规则
    -- ,t5.sub_page_id      as sub_page_id       -- 频道页
	,sub_page_ids
from
(
    select
...
-- on t3.uuid=t1.uuid
-- left join (
           -- select distinct uuid,is_free -- 1: 免费, 0:非免费(包括已解锁和未解锁)
