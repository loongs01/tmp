...
replace into  dwd_data.dwd_t01_reelshort_user_detail_info_di
 select      t1.uuid
            ...
            ,nvl(t10.first_visit_landing_page,'') as first_visit_landing_page -- 首次访问着陆页
            ,nvl(t10.story_id,'')                 as first_visit_landing_page_story_id -- 首次访问着陆页书籍id
            ,nvl(t10.chap_id,'')                  as first_visit_landing_page_chap_id  -- 首次访问着陆页章节id
from   (
...
left join (
           ...
           from
               ...
               ,min(stime) as stime
               ,case
                   when referrer_url like '%google%'     then '1'
                   when referrer_url like '%reelshort%' or referrer_url is null  then '2'
                   when referrer_url like '%facebook%'   then '3'
                   when referrer_url like '%youtube%'    then '4'
                   when referrer_url like '%bing%'       then '5'
                   else '6'
                end as recent_visit_source -- 最近一次访问来源
               ,case
                   when url like '%campaign_id%' then '2'
                   else '1'
