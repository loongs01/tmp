select
       ...
       -- ,cast(nvl(max(if(register_days=1,retention_uv)) / max(if(register_days=0,retention_uv)),0.0) as decimal(18,4)) as user_expose_retention_uv_1d -- 搜索次留
       -- ,cast(nvl(max(if(register_days=3,retention_uv)) / max(if(register_days=0,retention_uv)),0.0) as decimal(18,4)) as user_expose_retention_uv_3d -- 搜索3留
       -- ,cast(nvl(max(if(register_days=7,retention_uv)) / max(if(register_days=0,retention_uv)),0.0) as decimal(18,4)) as user_expose_retention_uv_7d -- 搜索7留
from
(
       select
       ...
       (
              ...
              from dwd_data.dwd_t80_reelshort_user_search_detail_di t1
              {{join_oth_dim}}
