        ...
        for idx, line in enumerate(collectlist):
            batch.append(ReplaceOne(
                {'analysis_date': line.get("analysis_date"),
                 'name_en_short': line.get("name_en_short"),
                 'dau_average_30d': line.get("dau_average_30d"),
                 'play_cnt_init': line.get("play_cnt_init"),
                 'favorite_cnt_init': line.get("favorite_cnt_init"),
                 'collect_cnt_init': line.get("collect_cnt_init")
                 }, line,
                upsert=True
            ))
            # print(batch)
...
if __name__ == '__main__':
    ...
    collection_name = 'story_play_init'
    date_str = sys.argv[1]
    # date_str="2023-09-19";
    # date_str="2023-10-17";
    ...
    sql = """select cast(t1.analysis_date as varchar) as analysis_date
                    ,t2.name_en_short
                    ,ceiling(t1.dau_average_30d) as dau_average_30d
                    ,t1.play_cnt_init
                    ,t1.favorite_cnt_init
                    ,t1.collect_cnt_init
                    from dm_reelshort_data.dm_t82_reelshort_story_play_init_di as t1
                    left join (select
                                  id
                                  ,split(replace(name_en_short,'_','-'),'-')[1] as name_en_short
                            from  chapters_log.language_info_v2
                            where name_en_short not like '%-%' and  name_en_short not like '%\_%'
                            group by id,split(replace(name_en_short,'_','-'),'-')[1]
                            having name_en_short not in('brazil','nb','nn','in','cn')
                            ) as t2
                    on t2.id=t1.language_id
                    where analysis_date="{}"
                    and t2.name_en_short is not null
