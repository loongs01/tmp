...
select
      ...
      ,coalesce(t10.show_cnt,0)                                            as show_cnt
      ,coalesce(t10.click_cnt,0)                                           as click_cnt
...
left join (
        ...
        from (
              ...
              select analysis_date
                    ...
                    ,cast(coalesce(json_extract(properties,'$.fcm_message_id'),json_extract(properties,'$.msg_id'),'') as varchar) as message_id
                    ,country_id                                                                                                        -- 国家id
              ...
              where analysis_date>=DATE_SUB('${TX_DATE}',INTERVAL 1 DAY)
                    ...
                    and app_id='cm1009'  -- App的唯一标识
                    and app_user_id<>''
              group by analysis_date,uuid,run_id,message_id,country_id,channel_id,version,language_id,platform
              ) t10
        -- 获取播放数据
        left join (
