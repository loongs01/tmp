...
left join (
        ...
        on t11.uuid = t10.uuid and t11.run_id = t10.run_id and t10.rn=1
        -- 获取内购付费数据
        left join (
                   select
                         distinct t1.uuid
        ...
        on t12.uuid = t10.uuid and t12.run_id=t10.run_id and t10.rn=1
        -- 获取订阅数据
        -- left join (
                   -- select
                         -- analysis_date
        ...
        left join (
                   select
                         ...
                         ,max(cast(nvl(json_extract(properties,'$._order_amount'),0) as int)) as sku_price -- 实际支付订单价格 (单位:分)
                   ...
                   group by app_user_id,app_activate_id
                  ) as t15
        on t15.uuid = t10.uuid and t15.run_id=t10.run_id and t10.rn=1
        -- 获取广告收入数据
        left join (
                    select
                    uuid
        ...
        on t14.uuid = t10.uuid and t14.run_id = t10.run_id and t10.rn=1
        -- 获取用户使用时长:秒
        left join (
                     select
                           uuid
        ...
        on t16.uuid = t10.uuid and t16.run_id = t10.run_id and t10.rn=1
) as t10
...
-- /*+ cte_execution_mode=shared */
-- insert into  dwd_data.dwd_t04_reelshort_opc_push_detail_di
,reelshort_push_stat as (select t.*
...
select
      ...
      ,null                                                               as content
      ,t1.sub_event_name                                                  as push_type
...
(
    ...
    group by analysis_date
             ...
             ,run_id
             ,analytics_label
             ,if(sub_event_name='custom_push_stat',fcm_message_id,coalesce(analytics_label,fcm_message_id))
             ,fcm_message_id
             ,platform
             ,country_id
             ,language_id
...
,t.uuid                              as uuid
,t.analytics_label                   as analytics_label
,t.analysis_date                     as analysis_date
,t.app_id                            as app_id
,t.instance_id                       as instance_id
,t.message_id                        as message_id
,t.create_channel                    as create_channel
,t.platform                          as platform
,t.title                             as title
,t.content                           as content
,t.push_type                         as push_type
,t.planned_quantity                  as planned_quantity            -- 计划发送数
,t.sent_quantity                     as sent_quantity               -- 已发送数
,t.last_send_time_at                 as last_send_time_at
,t.status                            as status
,t.is_success                        as is_success
,t.accepted_cnt                      as accepted_cnt
,t.delivered_cnt                     as delivered_cnt               -- 接收次数
,t.show_cnt                          as show_cnt                    -- 该消息id+uid+day是全屏intent，展示、点击、点击后转化类指标均取intent上报数据
,t.click_cnt                         as click_cnt
,t.user_type                         as user_type
,t.is_pay_lt                         as is_pay_lt                   -- 生命周期内购状态 :1-是，0-否
,t.vip_type_lt                       as vip_type_lt                 -- 生命周期订阅状态:1-是，0-否
,t.is_login                          as is_login
,t.country_id                        as country_id                  -- 国家id
,t.language_id                       as language_id                 -- 语言id
,t.channel_id                        as channel_id                  -- 渠道id
,t.version                           as version                     -- APP的应用版本(包体内置版本号versionname)
,t.cversion                          as cversion
,if(t1.uuid is not null,t1.is_play,t.is_play)                          as is_play                     -- 是否播放
,if(t1.uuid is not null,t1.play_cnt,t.play_cnt)                        as play_cnt                    -- 播放次数
,if(t1.uuid is not null,t1.is_pay,t.is_pay)                            as is_pay                      -- 是否内购
,cast(if(t1.uuid is not null,t1.sku_price,t.sku_price) as decimal(18, 4))                      as sku_price                   -- 内购支付订单价格
,if(t1.uuid is not null,t1.is_first_subscribe,t.is_first_subscribe)                            as is_first_subscribe          -- 是否首次订阅
,cast(if(t1.uuid is not null,t1.first_subscribe_sku_price,t.first_subscribe_sku_price) as decimal(18, 4))         as first_subscribe_sku_price   -- 首次订阅金额
,if(t1.uuid is not null,t1.is_subscribe,t.is_subscribe)                                        as is_subscribe                -- 是否订阅
,cast(if(t1.uuid is not null,t1.subscribe_price,t.subscribe_price) as decimal(18, 4))          as subscribe_price             -- 订阅单价
,cast(if(t1.uuid is not null,t1.ad_revenue,t.ad_revenue) as decimal(18, 4))                    as ad_revenue                  -- 广告收入
,if(t1.uuid is not null,t1.sum_online_times,t.sum_online_times)                                as sum_online_times            -- 用户使用时长:分钟
,if(t1.uuid is not null,1,0)            
from (select
            task_id
            ,uuid
            ,analytics_label
            ...
            ,min(last_send_time_at)    as last_send_time_at
            ,max(status)               as status
            ,max(is_success)           as is_success
            ...
            ,max(vip_type_lt)          as vip_type_lt
            ,max(is_login)             as is_login
            ,country_id
            ,language_id
            ,channel_id
            ,version
            ,cversion
            ,max(is_play)                       as is_play
            ,sum(play_cnt)                      as play_cnt
            ,max(is_pay)                        as is_pay
            ,sum(sku_price)                     as sku_price
            ,max(is_first_subscribe)            as is_first_subscribe
            ,max(first_subscribe_sku_price)     as first_subscribe_sku_price
            ,max(is_subscribe)                  as is_subscribe
            ,sum(subscribe_price)               as subscribe_price
...
left join (select
                ...
                ,max(is_pay)                             as is_pay
                ,max(sku_price)                          as sku_price
                ,max(is_first_subscribe)                 as is_first_subscribe
                ,max(first_subscribe_sku_price)          as first_subscribe_sku_price
                ,max(is_subscribe)                       as is_subscribe
                ,max(subscribe_price)                    as subscribe_price
                ,max(ad_revenue)                         as ad_revenue
                ,max(sum_online_times)                   as sum_online_times
           from custom_intent where push_type='intent_stat'
           group by
                  uuid
                  ,analytics_label
          ) as t1
on t1.uuid=t.uuid and t1.analytics_label=t.analytics_label
union all
select
null                                 as id
,t.task_id                           as task_id
...
,cast(if(t1.uuid is not null,t1.ad_revenue,t.ad_revenue) as decimal(18, 4))                    as ad_revenue                  -- 广告收入
,if(t1.uuid is not null,t1.sum_online_times,t.sum_online_times)                                as sum_online_times            -- 用户使用时长:分钟
,if(t1.uuid is not null,1,0)                                                                   as is_intent                   -- 是否全屏intent
from (select
            task_id
            ,uuid
            ,analytics_label
            ...
            ,sum(ad_revenue)                    as ad_revenue
            ,sum(sum_online_times)              as sum_online_times
     from custom_intent where push_type='custom_push_stat'
     group by
             task_id
             ,uuid
             ,analytics_label
...
on t1.uuid=t.uuid and t1.message_id=t.message_id
;