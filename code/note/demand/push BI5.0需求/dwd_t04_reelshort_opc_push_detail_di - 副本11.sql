...
,click_user_detail as (
                       ...
                       from dwd_data.dwd_t04_reelshort_opc_click_di -- 用户push点击明细
...
left join (
        ...
        from click_user_detail as t10 -- 获取点击数据
        -- 获取播放数据
        left join (
                  select t2.task_id,t1.uuid, count( distinct t1.stime) as play_cnt -- 播放次数
                  from dwd_data.dwd_t02_reelshort_play_event_di t1
				  inner join click_user_detail t2
				  on t2.uuid=t1.uuid
        ...
        left join (
                   ...
                   where is_abnormal=0
                         ...
                         and DATE_FORMAT(convert_tz(from_unixtime(end_time), 'Asia/Shanghai', 'America/Los_Angeles'),'%Y-%m-%d')='${TX_DATE}'
                   ) t13
